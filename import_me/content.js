...
(function () {
  ...
  // ─── SKIP HOSTS ────────────────────────────────────────────────────────────
  // Do not run on email clients, productivity apps, or other non-search sites
  // where broad selectors and text signals produce false positives.
  const SKIP_HOSTS = [
    // Email
    'mail.google.com',
    'mail.yahoo.com',
    ...
    'mail.aol.com',
    'inbox.google.com',
    // Docs / productivity (contain lots of text that triggers text signals)
    'docs.google.com',
    'sheets.google.com',
    'slides.google.com',
    'notion.so',
  ...
  // Bail out immediately — do not inject CSS, do not observe, do nothing.
  if (SKIP_HOSTS.some(h => location.hostname === h || location.hostname.endsWith('.' + h))) {
    return;
  }
  ...
  const SELECTORS = [
    ...
    // Covers: AI Answers strip, Ask Brave panel, Leo suggestion in search bar.
    // Source: Brave community confirmed selectors (stable across builds).
    '#submit-llm-button',                // "Answer with AI" button in search bar
    '.llm',                              // Stable Brave class on ALL LLM elements
    '[class*="llm-"]',                   // LLM sub-component class variants
    '[id="summarizer"]',                 // AI Summarizer wrapper (top of results)
    '[id*="summarizer"]',                // Summarizer sub-sections
    '[id="answer"][data-type]',           // AI Answers card (scoped by data-type)
    '[id="ask-brave"]',                  // "Ask Brave" chat panel
    '[data-type="summarizer"]',          // Summarizer type attribute
    '[data-testid="ai-answer"]',         // AI answer data-testid
    '.answer-header',                    // AI answer heading strip
    '[class*="answer-with-ai"]',         // Answer-with-AI container variants

    // ── Generic / Cross-Platform ─────────────────────────────────────────────
    '[class*="ai-generated"]',
    '[class*="aiGenerated"]',
    '[id*="ai-generated"]',
    '[data-ai-content]:not([class*="email"]):not([class*="message"])',
    '[data-ai-response]:not([class*="email"]):not([class*="message"])',
  ...
  function isProtectedContainer(el) {
    ...
    // Protect email-like containers: anything with email metadata attributes,
    // message thread roles, or common mail framework class patterns.
    if (el.closest('[role="main"][aria-label]')) return true;
    if (el.closest('[data-message-id]')) return true;
    if (el.closest('[data-thread-id]')) return true;
    if (el.closest('[data-legacy-message-id]')) return true;
    if (el.matches('[class*="message"],[class*="email"],[class*="thread"],[class*="inbox"]')) return true;
    // Protect form inputs and text editors
